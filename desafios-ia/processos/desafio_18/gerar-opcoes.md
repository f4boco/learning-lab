...
2. Criar uma função `listarOpcoes`:  
    ...
    2.3 Percorrer todos os elementos do vetor `meses` com `for()`;  
    2.4 Para cada `i`:  
        2.4.1 Criar um novo Elemento html `novaOpcao` que recebe uma `<option>`
        2.4.2 Definir o `novaOpcao.value` como as três primerias letras de `meses[i]`;  
        2.4.3 Definir o `novaOpcao.text` `como meses[i]`;  
        DECISÃO:  
        2.4.4 Verifica se `meses[i] === mesAtual`;  
            SIM:  
