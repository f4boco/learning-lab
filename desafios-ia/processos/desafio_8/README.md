# 🤖 Desafio: O Validador de Senhas Seguras

O usuário digita uma senha em um campo do formulário. Seu script deve validar se a senha atende aos seguintes requisitos mínimos de segurança simultaneamente:
    1. Deve ter entre 8 e 32 caracteres.
    2. Deve conter pelo menos um número.
    3. Deve conter pelo menos uma letra maiúscula.
    4. Deve conter pelo menos um símbolo.
    
Resultado: Exiba "Senha Forte" se passar nos testes, ou "Senha Insegura" caso falhe em algum.

---

## 🎯 Aprendizados Praticados
- Strings e funções strings;
- RegEx e métodos regex.text();
- Condicionais;
- Vetor de Objetos;
- Laço de repetição;

---

## 👨‍💻 Autor

Desenvolvido por **Fabiano O.**.